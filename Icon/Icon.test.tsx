...
describe('Icon Component', () => {
  ...
  describe('Accessibility', () => {
    it('renders as decorative with aria-hidden', async () => {
      const { container } = render(<Icon name="add" aria-hidden />);
    ...
    it('renders as decorative with empty aria-label', async () => {
      const { container } = render(<Icon name="add" aria-label="" />);
    ...
    it('uses custom aria-label when provided', async () => {
      const { container } = render(<Icon name="add" aria-label="Add item" />);
      ...
      await waitFor(
        () => {
          ...
          expect(wrapper).toHaveAttribute('aria-label', 'Add item');
    ...
    it('uses default label derived from name when no aria props provided', async () => {
      const { container } = render(<Icon name="arrow_back" />);
      ...
      await waitFor(
        () => {
          ...
          expect(wrapper).toBeInTheDocument();
          expect(wrapper).toHaveAttribute('role', 'img');
          expect(wrapper).toHaveAttribute('aria-label', 'arrow back');
          expect(wrapper).not.toHaveAttribute('aria-hidden');
        },
        { timeout: 3000 }
      );
    ...
    it('renders decorative during loading state', async () => {
      global.fetch = vi.fn().mockImplementation(() => new Promise(() => {}));

      let container: HTMLElement;
      await act(async () => {
        const result = render(<Icon name="loading" aria-hidden />);
        container = result.container;
        await new Promise((resolve) => setTimeout(resolve, 0));
      });
