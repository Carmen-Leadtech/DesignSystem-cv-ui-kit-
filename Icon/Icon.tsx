...
export interface IconProps extends IconSharedProps {
   ...
   * - Non-empty string: icon will be accessible with this label
   * - Empty string: icon will be treated as decorative (same as aria-hidden)
   * - Undefined: icon will use auto-generated label from name (default behavior)
   */
  'aria-label'?: string;
  /**
   * If true, hides icon from screen readers (decorative icon).
   * Alternative to aria-label="" for marking icons as decorative.
   */
...
export const Icon = ({
  ...
  const isDecorative = useMemo(() => {
    // Treat as decorative if aria-hidden is true or aria-label is empty string
    return ariaHiddenProp === true || ariaHiddenProp === 'true' || ariaLabelProp === '';
  }, [ariaHiddenProp, ariaLabelProp]);

  const accessibleLabel = useMemo(() => {
    if (isDecorative) return undefined;
    return ariaLabelProp || ariaLabel;
  }, [isDecorative, ariaLabelProp, ariaLabel]);
  ...
  const isReady = !!svg;
  if (error || !isReady) {
    const loadingErrorLabel = error ? `Error loading ${name}` : `Loading ${name}`;
    return (
      <span
        className={classNames(styles.root, className)}
        style={iconStyle}
        data-qa={dataQa || undefined}
        {...(!isDecorative && { role: 'img', 'aria-label': loadingErrorLabel })}
  ...
  if (color && maskDataUri) {
    return (
      <span
        ...
        style={maskStyle}
        data-qa={dataQa || undefined}
        {...(!isDecorative && { role: 'img', 'aria-label': accessibleLabel })}
        {...(isDecorative && { 'aria-hidden': true })}
      />
    );
  }
